...
class CuckooFilter {
  ...
  inline size_t AltIndex(const size_t index, const uint32_t tag) const {
    ...
    int alt_index = table_->NumBuckets() - index -
                    (((uint64_t)tag * 0x5bd1e995) % table_->NumBuckets());
    if ((((uint64_t)tag * 0x5bd1e995) % table_->NumBuckets()) == 0)
      alt_index -= 1;
    int alt_index_0 = table_->NumBuckets();
    int alt_index_1 = alt_index_0 - index;
    int val = (((uint64_t)tag * 0x5bd1e995) % table_->NumBuckets());
    int alt_index_2 = alt_index_1 - val;
    // if (alt_index < 0)
    //   std::cout << "yes" << std::endl;
    uint64_t alt_index_back = alt_index;
    alt_index += alt_index < 0 ? table_->NumBuckets() : 0;
    if (alt_index >= table_->NumBuckets())
      std::cout << "yes" << std::endl;
    return alt_index;
...
public:
  ...
  explicit CuckooFilter(const size_t max_num_keys)
      : num_items_(0), victim_(), hasher_(), seed1(0xc6a4a793),
        seed2(0x5bd1e995), n_hashchooser(16) {
    size_t assoc = 4;
    num_buckets = max_num_keys / 0.935 / assoc;
...
void CuckooFilter<ItemType, bits_per_item, TableType, HashFamily>::
  ...
  for (uint64_t i = 0; i < negatives.size(); i++) {
    ...
    uint64_t hash = hasher_(negative_key);
    hash >>= 32;
    size_t index = (hash * table_->NumBuckets()) >> 32;
...
Status CuckooFilter<ItemType, bits_per_item, TableType, HashFamily>::AddImpl(
  ...
  size_t curindex = i;
  uint32_t curtag = TagHash(item, 0);
  uint32_t oldtag;
...
void CuckooFilter<ItemType, bits_per_item, TableType, HashFamily>::adjust_hash(
  ...
  for (uint32_t j = 0; j < 4; j++) {
    ...
    if (!item)
      continue;
    uint32_t tag = TagHash(item, hash_chooser);
...
Status CuckooFilter<ItemType, bits_per_item, TableType, HashFamily>::Contain(
  ...
  hash >>= 32;
  i1 = (hash * table_->NumBuckets()) >> 32;
  tag = TagHash(key, 0);
  i2 = AltIndex(i1, tag);
  uint64_t hash_chooser1 = table_->GetHashChooser(i1);
  uint64_t hash_chooser2 = table_->GetHashChooser(i2);
  uint32_t tag1 = TagHash(key, hash_chooser1);
  uint32_t tag2 = TagHash(key, hash_chooser2);
