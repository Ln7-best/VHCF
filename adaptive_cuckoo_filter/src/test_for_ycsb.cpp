...
int run() {
  ...
  util::read_file("C:\\study\\papercode\\infocom24\\caida_raw.txt", negatives,
                  7000000);
  util::read_file("C:\\study\\papercode\\infocom24\\caida_weights_0.99.txt",
                  weights, 7000000);
  // for (uint64_t i = 0; i < weights.size(); i++)
  //   weights[i] = 1;
  std::cout << "reading complete" << std::endl;
  for (uint64_t bpk = 11; bpk <= 11; bpk++) {
    ...
    for (int loop = 0; loop < max_loop; loop++) {
      ...
      std::cout << insert_keys.size() << std::endl;
      for (int64_t i = 0; i < tot_i; i++) {
        // int64_t key= rand();
        // unsigned int key= (rand()*2^16)+rand();
        uint64_t key = insert_keys[i].val;
        S_map[key] = line++;
      ...
      */
      ...
      for (int64_t iter = 0; iter < ar_size; iter++) {
        ...
        if (flagFF) {
          ...
          pFF[false_i][p].second =
              fingerprint(key1, pFF[false_i][p].first, fbhs);
          num_swap++;
        }
      }
#ifdef offline
  ...
  for (uint64_t i = 0; i < costs.size(); i++)
    std::cout << costs[i] << ",";
  return 0;
}
