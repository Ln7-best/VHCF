...
class HTmap;  // forward declare
template <typename key_type, typename value_type>
class HTIterator;  // forward declare
...
class HTIterator {
 ...
 public:
  HTIterator(HTmap<key_type, value_type>& _ht, int _x, int _y,
             int _z)  // constructor
...
template <typename key_type, typename value_type>
HTIterator<key_type, value_type>&
HTIterator<key_type, value_type>::operator++() {
...
/*inline uint64 CityHash64WithSeed(std::pair<int64_t,int64_t> key, uint64_t
seed)
{
    char k[16];
...
HTmap<key_type, value_type>::HTmap(int way, int buckets, int hsize) {
  HTmap(way, buckets, hsize, 1000);
}

template <typename key_type, typename value_type>
HTmap<key_type, value_type>::HTmap() {
  HTmap(2, 4, 1024 * 1024, 1000);
}
...
HTmap<key_type,value_type>::expand()
{
  ...
  for (int i = 0;  i <K;  i++) {
      for (int ii = 0;  ii <b;  ii++){
          ...
          HTmap<key_type,value_type>::insert(temp_table[i][ii][iii]=
table[i][ii][iii]);
          }
      }
  }
 ...
 */
template <typename key_type, typename value_type>
bool HTmap<key_type, value_type>::direct_insert(key_type key, value_type value,
                                                int i, int ii) {
...
value_type HTmap<key_type, value_type>::query(key_type key) {
  ...
  for (int i = 0; i < K; i++) {
    for (int ii = 0; ii < b; ii++) {
      int p = myhash<key_type>(key, i, m);
      // verprintf("query item in table[%d][%d] for p=%d and
      // f=%d\n",p,jj,p,fingerprint); verprintf("result is: %d\n",table[p][jj]);
      if ((present_table[i][ii][p]) && (table[i][ii][p].first == key)) {
        return table[i][ii][p].second;
      }
    }
 ...
 */
template <typename key_type, typename value_type>
tuple<value_type, int, int, int, int> HTmap<key_type, value_type>::fullquery(
    key_type key) {
  int num_lookup = 0;
  if ((key == victim_key) && (victim_flag))
    return std::make_tuple(victim_value, -2, -2, -2, 0);
  for (int i = 0; i < K; i++) {
    for (int ii = 0; ii < b; ii++) {
      num_lookup++;
      int p = myhash<key_type>(key, i, m);
      // verprintf("query item in table[%d][%d] for p=%d and
      // f=%d\n",p,jj,p,fingerprint); printf("query item in
      // table[%d][%d][%d]=%d\n",i,ii,p); verprintf("result is:
      // %d\n",table[p][jj]);
      if ((present_table[i][ii][p]) && (table[i][ii][p].first == key)) {
