...
public:
  ...
  BloomFilter() {}
  explicit BloomFilter(const size_t n, size_t bits_per_item, bool branchless)
      : hasher() {
    this->bits_per_item = bits_per_item;
    this->branchless = branchless;
    this->size = 0;
    this->kk = getBestK(bits_per_item);
    k = (int)(((double)bits_per_item) * 0.693147180559945 + 0.5);
    this->bitCount = n * ((double)bits_per_item);
...
private:
  ...
  size_t bits_per_item_float_num;
  bool branchless;
};

template <typename ItemType, typename HashFamily>
Status BloomFilter<ItemType, HashFamily>::Add(const ItemType &key) {
...
template <typename ItemType, typename HashFamily>
Status BloomFilter<ItemType, HashFamily>::AddAll(const ItemType *keys,
                                                 const size_t start,
                                                 const size_t end) {
...
template <typename ItemType, typename HashFamily>
Status BloomFilter<ItemType, HashFamily>::Contain(const ItemType &key) const {
  uint64_t hash = hasher(key);
  uint64_t a = (hash >> 32) | (hash << 32);
  uint64_t b = hash;
  if (branchless && k >= 3) {
...
template <typename ItemType, typename HashFamily>
std::string BloomFilter<ItemType, HashFamily>::Info() const {
  std::stringstream ss;
...
template <size_t blocksize, int k, typename HashFamily>
inline bool SimpleBlockFilter<blocksize, k, HashFamily>::Find(
