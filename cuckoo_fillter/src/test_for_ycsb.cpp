...
int main() {
  ...
  */
  ...
  for (uint64_t k = 0; k < 100; k++) {
    ...
    if (fail_flag)
      continue;
    ycsbc::Generator<uint64_t> *key_chooser_ =
        new ycsbc::ScrambledZipfianGenerator(_min, _max, theta);
    for (uint64_t i = 0; i < 1000000; i++) {
      count++;
      uint64_t lookup_key = key_chooser_->Next();
  ...
  std::cout << (double)fp_count / (100 - fail_count) / 1000000 << ",";
  fp_count = 0;
  for (uint64_t k = 0; k < 100; k++) {
    bool fail_flag = false;
